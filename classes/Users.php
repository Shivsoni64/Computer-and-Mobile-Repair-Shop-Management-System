...
class Users extends DBConnection
{
	...
	public function save_users()
	{
		...
		if (isset($oldpassword)) {
			if (md5($oldpassword) != $this->settings->userdata('password')) {
				return 4; // Old password does not match
			}
		}
		...
		if (isset($_FILES['img']) && $_FILES['img']['tmp_name'] != '') {
			...
			if (!in_array($type, $allowed)) {
				...
				$fname = 'uploads/avatar-' . $id . '.' . $ext; // Dynamic file name
				...
				if ($gdImg) {
					...
					if ($type == 'image/png') {
						...
						$uploaded_img = imagejpeg($t_image, $dir_path);
					}
					imagedestroy($gdImg);
	...
	public function save_client()
	{
		...
		// Handle image upload for client
		if (isset($_FILES['img']) && $_FILES['img']['tmp_name'] != '') {
			$upload = $_FILES['img']['tmp_name'];
			$type = mime_content_type($upload);
			$allowed = array('image/png', 'image/jpeg');
			...
			if (!in_array($type, $allowed)) {
				...
				// Determine file extension
				$ext = ($type == 'image/png') ? 'png' : 'jpg';
				$fname = 'uploads/client-avatar-' . $id . '.' . $ext; // Dynamic file name
				$dir_path = base_app . $fname;
